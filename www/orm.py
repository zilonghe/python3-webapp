...
class ModelMetaclass(type):
	...
	escaped_fields = list(map(lambda f: "`%s`", fields))#转义
	...
	attrs['__update__'] = "update `%s` set `%s` = `%s` where `%s` = %s" %(tableName, ', '.join(map(lambda f: "`%s`" % (mappings.get(f).name or f), fields)), primarykey)
